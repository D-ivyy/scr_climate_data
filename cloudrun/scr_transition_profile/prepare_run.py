#!/usr/bin/env python3
"""Freeze and optionally stage one SCR transition-risk Cloud Run inventory."""
...
def main() -> None:
    ...
    output = run_text(["gcloud", "storage", "ls", "--recursive", f"{source_prefix}**"])
    source_uris = sorted(
    ...
    if args.stage:
        for offset in range(0, len(source_uris), CHUNK_SIZE):
            ...
            if completed.returncode:
                ...
                raise RuntimeError(f"Staging failed for source offset {offset}: {detail}")
            print(
                json.dumps(
                    {
    ...
    inventory = {
        "schema_version": "scr_transition_source_inventory_v1",
